...
def get_alignconsts_errors(filename, tablename):
    ...
    zrot = []
    length = 36
    if tablename == "TrkAlignPanel":
      length = 216
    for plane in range(0,length):
        x_shift_errors.append(float(alignconsts.tables[tablename].errors[plane][0]))
        y_shift_errors.append(float(alignconsts.tables[tablename].errors[plane][1]))
        z_shift_errors.append(float(alignconsts.tables[tablename].errors[plane][2]))

        xrot.append(float(alignconsts.tables[tablename].errors[plane][3]))
        yrot.append(float(alignconsts.tables[tablename].errors[plane][4]))
        zrot.append(float(alignconsts.tables[tablename].errors[plane][5]))
...
#for param_col in [1,2]:
#    plt.figure(param_col)
    to_plot = []
    col_str = ['x-shift', 'y-shift','z-shift', 'x-rot','y-rot','z-rot'][param_col]
    for label, filename in pairwise(sys.argv[1:]):
        shifts_errors = get_alignconsts_errors(filename, 'TrkAlignPanel')
        shifts = get_alignconsts(filename, 'TrkAlignPanel')
        shifts = shifts[:,param_col]*1000 # convert to micrometers
        #shifts_errors = np.zeros((216,))
...
#    with plt.style.context(['science', 'grid','no-latex']):
    ...
    # shifts per plane and pulls underneath
        ...
        axs[0].axhline(0,0,216, color='k',label='Nominal')
        ...
        for filename, shifts, shifts_errors in to_plot:
            ...
            axs[1].bar(np.arange(0,215.5,1), pulls,label=filename)
        ...
        axs[1].set_xticks(np.arange(0,216,2))
        axs[0].set_xticks(np.arange(0,216,2))
